                    ...
                    for (const mutation of mutationsList) {
                        if (window.location.href.indexOf('voxi.co.uk/acquisition/checkout') > -1) {
                            ...
                            if (mutation.target.matches('[data-testid="step-content"]') || mutation.target.matches('[data-testid="step"]') && vt654_headers.length > 7 && !vt654_spinner) {
                                ...
                                // //remove back btn//
                                var backBtn = document.querySelector('[class*="LinkWithIconstyle__LinkWithIconText-sc"]')
                                if (backBtn) {
                                    backBtn.parentElement.parentElement.remove()
                                }
                                //insert new btn
                                var yourOrderContainer = document.querySelector('[id="order-summary"]')
                                var croCTA = document.querySelector('.cro-link-container')
                                if (yourOrderContainer && !croCTA) {
                                    var vt767Target = yourOrderContainer.querySelector('ul')
                                    var vt767Link = '<div class="cro-link-container">    <div class="cro-link-content">        <a class="cro-link" href="https://www.voxi.co.uk/sim-only-plans">            Change plan        </a>    </div></div>'
                                    vt767Target.insertAdjacentHTML('afterend', vt767Link)
                                }
                                // //remove back btn -- END//
